...
static int recursive_truncate_meta_blocks(struct super_block *sb, __le64 block,
	u32 height, unsigned short btype, unsigned long first_blocknr,
	unsigned long last_blocknr, unsigned long start_pgoff,
	bool *meta_empty)
{
	unsigned long first_blk, last_blk, page_addr;
...
void pmfs_free_mem_addr(struct super_block *sb, __le64 addr, u32 btype)
{
	...
	if (pair->nvmm_entry) {
		...
		entry->invalid_pages++;
		nvmm = get_nvmm(sb, pair, pair->pgoff);
		pmfs_free_data_blocks(sb, nvmm, 1, btype);
		pair->nvmm_entry = 0;
	}
	...
	struct scan_bitmap *bm, unsigned long pgoff)
{
	...
	leaf->pgoff = pgoff;
	nvmm = (data->block >> PAGE_SHIFT) + pgoff - data->pgoff;
	if (bm) {
	...
	bool free, bool alloc_dram)
{
	...
	unsigned long pgoff, nvmm_block;
	unsigned int first_index, last_index;
	struct pmfs_file_write_entry *entry;
	struct mem_addr *leaf;
//	unsigned int flush_bytes;
//	struct pmfs_range_node *hint = NULL;
	...
	for (i = first_index; i <= last_index; i++) {
		if (height == 1) {
			...
			if (leaf->nvmm_entry && nvmm) {
				nvmm_block = get_nvmm(sb, leaf, leaf->pgoff);
				if (bm)
					clear_bm(nvmm_block, bm, BM_4K);
				if (free) {
					entry = pmfs_get_block(sb,
							leaf->nvmm_entry);
					entry->invalid_pages++;
					pmfs_free_data_blocks(sb, nvmm_block,
						1, pi->i_blk_type);
	...
	if (!sih->root) {
		...
		if (height == 0) {
			...
			if (root->nvmm_entry && nvmm) {
				...
				struct pmfs_file_write_entry *entry;
				unsigned long nvmm_block;

				nvmm_block = get_nvmm(sb, root, root->pgoff);

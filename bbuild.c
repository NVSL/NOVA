...
static void pmfs_init_blockmap_from_inode(struct super_block *sb)
{
	...
	struct pmfs_inode *pi =  pmfs_get_inode_by_ino(sb, PMFS_BLOCKNODE_IN0);
	struct pmfs_blocknode_lowhigh *p = NULL, *entry;
	...
	for (i = 0; i < num_blocknode; i++) {
		...
		entry = (struct pmfs_blocknode_lowhigh *)pmfs_get_block(sb,
							curr_p);
		if (entry->block_low != blknode->block_low ||
				entry->block_high != blknode->block_high)
			pmfs_dbg("%s: entry blocknode %lu mismatch, "
				"%lu %lu, %llu %llu\n", __func__, i,
				blknode->block_low, blknode->block_high,
				entry->block_low, entry->block_high);
...
void pmfs_save_blocknode_mappings(struct super_block *sb)
{
	...
	/* Move this to correct place later */
...
static int pmfs_recover_inode(struct super_block *sb, struct pmfs_inode *pi,
	u64 pi_addr, unsigned long ino, struct scan_bitmap *bm, int cpuid,
	int multithread)
{
	struct pmfs_inode_info_header *sih;
...
static inline struct pmfs_inode *task_ring_dequeue(struct super_block *sb,
	struct task_ring *ring,	unsigned long *ino, u64 *pi_addr)
{
	struct pmfs_inode *pi;
...
static void pmfs_inode_table_multithread_crawl(struct super_block *sb,
	struct pmfs_inode *pi, struct scan_bitmap *bm, int cpus)
{
	struct pmfs_sb_info *sbi = PMFS_SB(sb);
...
int pmfs_multithread_recovery(struct super_block *sb, struct scan_bitmap *bm)
{
	...
	pmfs_inode_table_multithread_crawl(sb, pi, bm, cpus);
